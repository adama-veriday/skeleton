...
1. Go to widget source (<MICROSERVICE_FOLDER>/ui/widgets/sub/formWidget/>):
   1. Run `npm i`
...
- **sub.form.createError** (fired when an error occurs during sub creation)
- **sub.form.create** (fired when the sub is successfully created)
- **sub.form.updateError** (fired when an error occurs during sub update)
- **sub.form.update** (fired when the sub is successfully updated)
