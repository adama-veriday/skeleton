...
describe('CustomerAddFormContainer', () => {
  ...
  it('saves data', async () => {
    ...
    const partner_idField = await findByLabelText('entities.customer.partner_id');
    fireEvent.change(partner_idField, { target: { value: customerMock.partner_id } });
    const productsField = await findByLabelText('entities.customer.products');
    fireEvent.change(productsField, { target: { value: customerMock.products } });
